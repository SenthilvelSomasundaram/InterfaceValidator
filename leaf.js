...
config false;
   ...
   if(type.indexOf('leafref') > -1 && (config==null || config=='') ){
		...
		type = type.replace("core-model:logical-termination-point","core-model:logical-termination-point[core-model:uuid=current()/../../../../core-model:uuid]");
		type = type.replace("core-model:layer-protocol","core-model:layer-protocol[core-model:local-id=current()/../../../core-model:local-id]");
            }
        }
